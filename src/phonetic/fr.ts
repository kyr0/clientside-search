...
type Preprocessing = [RegExp, string][]

const FIRST_PREPROCESSING: Preprocessing = [
  [/O[O]+/g, 'OU'],
  [/SAOU/g, 'SOU'],
  [/OES/g, 'OS'],
  [/CCH/g, 'K'],
...
const SECOND_PREPROCESSING: Preprocessing = [
  [/OIN[GT]$/g, 'OIN'],
  [/E[RS]$/g, 'E'],
  [/(C|CH)OEU/g, 'KE'],
  [/MOEU/g, 'ME'],
...
const FIRST_ENDINGS: Preprocessing = [
  [/USIL$/, 'USI'],
  [/X$|[TD]S$|[DS]$/, ''],
  [/([^KL]+)T$/, '$1'],
...
const SECOND_ENDINGS: Preprocessing = [
  [/TIL$/, 'TI'],
  [/LC$/, 'LK'],
  [/L[E]?[S]?$/, 'L'],
  [/(.+)N[E]?[S]?$/, '$1N'],
...
export function phonetic(word: string) {
  if (typeof word !== 'string') throw Error('the given word is not a string.')
